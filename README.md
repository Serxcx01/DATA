...
local index = 1
for nomor, bb in pairs(getBots()) do
...
local function PICK_ASSIST_WORLD(mode)
  ...
  -- mode "stale": hanya eligible kalau sudah melewati STALE_SEC, dan STEAL_HELP diizinkan
  if tostring(mode or ""):lower() == "stale" then
    if best_age >= (STALE_SEC or 1800) and (STEAL_HELP ~= false) then
      -- beri sinyal: kandidat ini boleh di-steal; eksekusi steal/commit dilakukan di luar
      return best, true
    else
      -- belum stale cukup → jangan assist world ini
      return nil, false
    end
  end

  -- mode "always": bantu tanpa steal (owner tetap), limit sudah dicek
  return best, false
end
...
local function _try_commit_helper(world)
  ...
  -- (A) buang baris lama milik kita di world ini (anti-duplikat)
  do
  ...
  -- (B) APPEND claim helper (atomik per-world karena pakai lock)
  local fh = io.open(JOB_FILES.inprogress, "a")
  if not fh then
    print(string.format("[COMMIT] %s gagal open inprogress untuk %s", WORKER_ID, world))
  ...
  if helpers_now > (ASSIST_HELPER_LIMIT or 1) then
    ...
    _write_lines(JOB_FILES.inprogress, out)
    print(string.format("[COMMIT] %s rollback %s (helpers=%d > limit=%d)",
      WORKER_ID, world, helpers_now, (ASSIST_HELPER_LIMIT or 1)))
    ok = false
  else
    ok = true
    print(string.format("[COMMIT] world=%s helpers_now=%d limit=%d ok=%s",
      world, helpers_now, (ASSIST_HELPER_LIMIT or 1), tostring(ok)))
  end
