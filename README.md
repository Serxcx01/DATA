...
-- + SMART MULTI-BOT ASSIGNMENT (AUTO SLOT & AUTO TOTAL_BOTS)
-- + WARP TAHAN BANTING (NUKED + PUBLIC + BAD_DOOR + SAFE TILE)
-- + LOGGING WORLD GAGAL (warp_fail.log)
-- + GUARD: ANTI-STUCK kalau respawn tepat di WHITE DOOR (fg==6)
-- + TXT JOB QUEUE (worlds.txt / inprogress.txt / done.txt) + WORK-STEALING
----------------------------------------------------------------

-- BASE PATH KE DESKTOP (Windows) + PATH HELPERS
----------------------------------------------------------------
...
-- join path aman (karena extraFilePath sudah pakai trailing "/")
...
STEAL_HELP    = true         -- idle bot boleh bantu job lain (lihat gating di bawah)
STALE_SEC     = 30 * 60      -- job dianggap macet jika heartbeat > 30 menit

-- ARAHKAN FILE QUEUE/LOG KE DESKTOP
JOB_FILES = {
...
-- Storage CAKE (global)
STORAGE_CAKE, DOOR_CAKE = "GUDANGARIZ1", "AKUKEREN"
...
-- ===== GUARD: anti stuck di white door =====
...
for nomor, bot in pairs(getBots()) do
  if getBot().name:upper() == bot.name:upper() then index = nomor end
end
...
-------------------- POSISI & OBJECT (DROP snake cache) --------------------
  ...
  for _, ITEM in pairs(ITEMS) do
    ...
    while have > 0 do
      ...
      if not _gotoExact(WORLD, DOOR, stanceX, stanceY, PATH_TRY, STEP_MS) then
        mark_bad(candX, candY); cursor.x = candX + 1; goto seek_slot
      end
      faceSide2()

      local total, stacks = _countOnTile(candX, candY)
      if (total >= TILE_CAP) or (stacks >= STACK_CAP) then
        mark_full(candX, candY); cursor.x = candX + 1; goto seek_slot
      end
...
-- warp + cek nuked + cek public + cek door (logging)
...
-------------------- TAKE MAGNI (10158) — diperbaiki: keep 1, sisanya drop ke storage --------------------
-- helper: drop ke storage hingga sisa "keep"
...
function TAKE_MAGNI(WORLD, DOOR)
  ...
  -- ===== [EARLY GUARD] kalau di BP sudah >1, sisakan 1 lalu RETURN =====
  do
    ...
    if have > 1 then
      ...
      -- opsional: pastikan terpakai
      if inv:getItemCount(TARGET_ID) > 0 then b:wear(TARGET_ID); sleep(300) end
      return true
    end
  end

  -- (0) Ambil dulu kalau belum punya sama sekali
  ...
  -- (1) Normalisasi: sisakan tepat 1; jika lebih (mis. kebetulan pick double), drop ke STORAGE_MAGNI
  local storageW, storageD = STORAGE_MAGNI, DOOR_MAGNI
  if (storageW or "") == "" then storageW, storageD = WORLD, DOOR end
  _ensure_single_item_in_storage(TARGET_ID, 1, storageW, storageD,
    {chunk=200, step_ms=600, path_try=10, max_cols=50, max_rows=20, tile_cap=4000, stack_cap=20, tile_retries=2})
...
local function HARVEST_PASS(FARM_WORLD, FARM_DOOR, farmListActive)
  ...
  if USE_MAGNI then
    ...
    if have == 0 then
      if (STORAGE_MAGNI or "") ~= "" then
        TAKE_MAGNI(STORAGE_MAGNI, DOOR_MAGNI)  -- TAKE_MAGNI akan wear DI DALAMNYA
        called_take = true
      end
      if inv:getItemCount(10158) == 0 then
        TAKE_MAGNI(FARM_WORLD, FARM_DOOR)      -- TAKE_MAGNI akan wear DI DALAMNYA
        called_take = true
      end

    elseif have > 1 then
      -- normalisasi ke 1 + drop sisanya; TAKE_MAGNI akan wear DI DALAMNYA
      TAKE_MAGNI(FARM_WORLD, FARM_DOOR)
      called_take = true
    end

    -- Jika dari awal sudah tepat 1 (tidak memanggil TAKE_MAGNI), barulah wear sekali di sini
    if (not called_take) and inv:getItemCount(10158) > 0 then
      bot:wear(10158)  -- dipanggil SATU kali saja
      sleep(200)
    end
  end
...
-- on_tick optional untuk heartbeat queue
...
----------------------------------------------------------------
-- ============ TXT QUEUE + WORK-STEALING (dengan gating) =============
----------------------------------------------------------------
...
-- statistik queue buat gating assist
...
-- Drop tanpa ambang minimal: asal ada salah satu item cake
local function has_any_cake()
  local b = getBot and getBot() or nil; if not (b and b.getInventory) then return false end
  local inv = b:getInventory()
  for _, id in pairs(cakeList or {}) do if inv:getItemCount(id) > 0 then return true end end
...
function RUN_FROM_TXT_QUEUE()
  -- AUTO-RESUME: lanjutkan job milik sendiri jika ada
  ...
  while true do
    ...
    if job then
      ...
      -- 2) Tidak ada job baru yang bisa diklaim
      --    -> GATING ASSIST: hanya bantu jika TIDAK ADA unclaimed world lagi
      local qs = QUEUE_STATS()
      if qs.unclaimed > 0 then
        -- masih banyak world belum diklaim: JANGAN assist dulu
        print(string.format("[QUEUE] Masih ada %d world belum diklaim. Menunggu...", qs.unclaimed))
        sleep(1000)
      else
        -- semua world sudah diklaim (atau selesai) -> boleh assist world yang stale
        ...
        if assistW then
          ...
          for _, ln in ipairs(worlds) do
            ...
            if W == assistW then
              ...
              _update_heartbeat(W, WORKER_NAME)
              HARVEST_UNTIL_EMPTY(W, D, SW, SD, {ITEM_BLOCK_ID, ITEM_SEED_ID}, hb)
              -- tidak mark done jika bukan klaimer awal, KECUALI kita sudah steal (owner==kita) — handled di PICK_ASSIST_WORLD
              -- (opsi: verifikasi owner==kita dan MARK_DONE)
              local rows = _read_lines(JOB_FILES.inprogress)
              local owner_now = nil
  ...
  -- FINAL CLEANUP: drop cake kalau ada (tanpa ambang), keluar world, auto_reconnect on
