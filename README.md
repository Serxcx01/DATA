...
-- - TAKE_MAGNI cooldown only after consecutive failures (log by STORAGE)
-- - DROP_ITEMS_SNAKE (tile/stack caps, stance check, snake scan)
-- - TXT queue runner RUN_FROM_TXT_QUEUE with helper limit & heartbeat TTL
-- - All reconnect/guard calls explicit
-- - hasAccess(x,y) global
----------------------------------------------------------------

-------------------- CONFIG (user-adjustable) --------------------
-- Assist modes: "stale" or "always"
ASSIST_MODE          = (ASSIST_MODE or "always")
ASSIST_MODE          = tostring(ASSIST_MODE):lower()
ASSIST_HELPER_LIMIT  = ASSIST_HELPER_LIMIT or 1  -- max helpers per world
STEAL_HELP           = (STEAL_HELP ~= false)     -- only relevant in stale mode
STALE_SEC            = STALE_SEC or 30 * 60      -- heartbeat TTL in seconds
LOOP_MODE            = LOOP_MODE or false        -- keep looping queue
DELAY_EXE            = DELAY_EXE or 1000         -- loop delay in ms
...
-- Item IDs (default; may be overridden per world via queue)
...
-- Drop snake limits (fallback bounds)
...
local function checkitemfarm(farmList)
  ...
  for _,item in pairs(farmList or {}) do if inv:getItemCount(item)>=190 then return true end end
  return false
end
local function has_any_item_farm(farmList)
  local inv=getBot():getInventory()
  for _,item in pairs(farmList or {}) do if inv:getItemCount(item)>0 then return true end end
  return false
end
...
-------------------- TXT-QUEUE RUNNER (with helper limit & TTL) --------------------
...
local function _parse_world_line(ln)
  ...
  local DOOR    = tostring(t[2] or ""):upper()
  local BLOCKID = tonumber(t[3] or "") or ITEM_BLOCK_ID
  local STOREW  = tostring(t[4] or ""):upper()
  local STORED  = tostring(t[5] or ""):upper()
...
local function _pick_job_from_worlds()
  ...
  for _, ln in ipairs(rows) do
    ...
    if WORLD then
      ...
      if ok then
        return {WORLD=WORLD, DOOR=DOOR, BLOCKID=BLOCKID, STOREW=STOREW, STORED=STORED}
      end
    end
  end
  return nil
...
-- farmListActive: list of item IDs considered "farm items" for storage drop
-- worker_id: unique per bot (e.g., bot name)
function RUN_FROM_TXT_QUEUE(worker_id, farmListActive)
  ...
  -- override item IDs per job if provided
