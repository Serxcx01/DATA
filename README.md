...
function ensureMalady(threshold_min, opts)
    ...
    local status_malady, total_secs, name_malady = checkMalady()
    local ttl_secs = tonumber(total_secs or 0) or 0

    -- 1) Belum ada malady → ambil sekarang
    if not status_malady then
        print("[MALADY] Belum ada malady → coba ambil di storage.")
        local okCall, okTake = pcall(take_malady, useW, useD, { step_ms = 650, rewarp_every = 180 })
        if okCall and okTake then
        print("[MALADY] Berhasil ambil.")
        return true, "taken"
        else
        local why = (not okCall) and "take_exception" or "take_failed"
        print("[MALADY] Gagal ambil:", why)
        return false, why
        end
    end

    -- 2) Sudah ada malady → putuskan tunggu/skip
    if ttl_secs > THRESH_SECS then
        print(("[MALADY] %s (%02dh %02dm %02ds) ~%ds > %d → skip.")
        :format(name_malady or "Unknown",
                tonumber(maladyHours or 0) or 0,
                tonumber(maladyMins  or 0) or 0,
                tonumber(maladySecs  or 0) or 0,
    ...
    local function fmt_hms(secs)
        secs = math.max(0, tonumber(secs or 0) or 0)
    ...
    -- 3) sisa ≤ threshold → nunggu selesai
    ...
    while ttl_secs > 0 do
        ...
        if (elapsed % recheck_every) == 0 then
        local ok, new_secs, new_name = checkMalady()
        if ok then
            ttl_secs    = tonumber(new_secs or ttl_secs) or ttl_secs
            name_malady = new_name or name_malady
        end
        end
    end
    print("[MALADY] Selesai (0s). Cek & coba ambil lagi...")

    -- sinkronisasi singkat: kadang /status masih sisa 1–2 detik
    local sync_deadline = os.time() + 10
    local ok2, secs2, name2 = checkMalady()
    while ok2 and tonumber(secs2 or 0) > 0 and os.time() < sync_deadline do
    sleep(1000)
    ok2, secs2, name2 = checkMalady()
    end

    -- retake jika tdk ada malady atau sisa <= 0
    if (not ok2) or (tonumber(secs2 or 0) <= 0) then
    print("[MALADY] Buff habis → ambil ulang di storage.")
    local okCall, okTake = pcall(take_malady, useW, useD, { step_ms = 650, rewarp_every = 180 })
    if okCall and okTake then
        print("[MALADY] Re-apply berhasil.")
        return true, "retaken"
    else
        local why = (not okCall) and "retake_exception" or "retake_failed"
        print("[MALADY] Re-apply gagal:", why)
        return false, why
    end
    else
    -- Kalau sampai sini, berarti masih terdeteksi malady dengan sisa > 0 (mis: sudah aktif lagi)
    print(("[MALADY] Sudah aktif lagi: %s, sisa ~%ds.")
            :format(name2 or "Unknown", tonumber(secs2 or 0) or 0))
