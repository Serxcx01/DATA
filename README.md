...
-- + AUTO-RESUME (berdasar SLOT), ASSIST "stale"/"always"
-- + WARP TAHAN BANTING (NUKED + PUBLIC + BAD_DOOR + SAFE TILE)
-- + DROP SNAKE ADAPTIF (cek fg walkable, sweep kanan->kiri), cache objek
-- + LOGGING WORLD GAGAL/SUKSES, GUARD stuck di white door
----------------------------------------------------------------
...
STEAL_HELP    = true            -- izinkan bantu job lain
ASSIST_MODE   = ASSIST_MODE or "always"  -- "stale" atau "always"
LOOP_MODE     = true            -- true: loop idle (reconcile, drop cake, leave)
...
-- Storage MAGNI
STORAGE_MAGNI, DOOR_MAGNI = "HVKKLL", "XX2" -- 10158

-- (MODE RR/CHUNK kalau USE_TXT_QUEUE=false)
LIST_WORLD = {
  -- "FARM_WORLD|FARM_DOOR|ITEM_BLOCK_ID|STORAGE_WORLD|STORAGE_DOOR"
}

-- File queue/log diarahkan ke folder
JOB_FILES = {
...
-- ================= MULTI-BOT (untuk RR/CHUNK dan SLOT) =================
TOTAL_BOTS   = 8
...
for nomor, bt in pairs(getBots()) do
  if getBot().name:upper() == bt.name:upper() then index = nomor end
end
...
-- Worker pakai SLOT
local WORKER_ID = "SLOT"..tostring(MY_SLOT or 1)
...
local function UPDATE_DELAY_BY_PING()
...
local function _tile_at(tx,ty)
  local b = getBot and getBot() or nil
  if not (b and b.getWorld) then return nil end
  local w = b:getWorld()
  return (w and w.getTile and w:getTile(tx,ty)) or nil
end

local function _is_walkable(tx,ty)
  local t = _tile_at(tx,ty)
  if not t then return false end
  -- dasar: fg==0 dianggap bisa diinjak
  if (t.fg or 0) ~= 0 then return false end
  -- optional: bisa tambah cek lava/spike jika engine expose
  return true
end
...
local function ZEE_COLLECT(state)
...
local function log_fail(world, door, reason)
  local f, err = io.open(FAIL_LOG_FILE, "a"); if not f then print("[LOG] Gagal buka fail log: "..tostring(err)); return end
...
local function _countOnTile(tx, ty)
...
-- ================= NEXT TILE (ADAPTIVE SNAKE) =================
local function _nextDropTileAdaptive(sx, sy, cursor, max_cols, max_rows, world_width, tile_cap, stack_cap, dir)
  ...
  local left_bound = math.max(0, sx - max_cols)
  local right_bound= math.min(world_width, sx + max_cols)
  dir = dir or "right"
  ...
  while tried_rows < max_rows do
    ...
    local stance_ok = (stanceX >= 0) and _is_walkable(stanceX, stanceY)
    local tile_ok   = _is_walkable(cx, cy)

    if stance_ok and tile_ok and (not is_full_or_bad(cx, cy)) then
      local total, stacks = _countOnTile(cx, cy)
      if (total < MAX_STACK) and (stacks < MAX_SLOTS) then
        cursor.x, cursor.y = cx, cy
        return cx, cy, cursor, dir
      else
        mark_full(cx, cy)
      end
    else
      if (not stance_ok) or (not tile_ok) then mark_bad(cx, cy) end
    end

    -- geser horizontal
    if dir == "right" then
      cx = cx + 1
      if cx > right_bound then
...
function DROP_ITEMS_SNAKE(WORLD, DOOR, ITEMS, opts)
  ...
  local MAX_COLS   = opts.max_cols or 99       -- sesuai request
  local MAX_ROWS   = opts.max_rows or 23       -- sesuai request
  ...
  for _, ITEM in pairs(ITEMS) do
    ...
    while have > 0 do
      ::seek_slot::
      local candX, candY
      candX, candY, cursor, dir = _nextDropTileAdaptive(sx, sy, cursor, MAX_COLS, MAX_ROWS, WORLD_W, TILE_CAP, STACK_CAP, dir)
      if not candX then print("[DROP] Area penuh / tidak ada tile walkable dalam jangkauan."); return end

      local stanceX, stanceY = candX - 1, candY
      if stanceX < 0 or (not _is_walkable(stanceX, stanceY)) then
        mark_bad(candX, candY); goto seek_slot
      end
      ...
      if (total >= TILE_CAP) or (stacks >= STACK_CAP) then
        mark_full(candX, candY); goto seek_slot
      end

      local cap      = math.max(0, TILE_CAP - total)
      local drop_try = math.min(have, CHUNK, cap)
      if drop_try <= 0 then mark_full(candX, candY); goto seek_slot end
      ...
      while drop_try > 0 and have > 0 do
        ...
        sleep(STEP_MS)
        SMART_RECONNECT(); GUARD_DOOR_STUCK()
        local after = b:getInventory():getItemCount(ITEM)

        if after < before then
          have = after
          local t2, s2 = _countOnTile(candX, candY)
          if (t2 >= TILE_CAP) or (s2 >= STACK_CAP) then mark_full(candX, candY); break end
          local sisa = TILE_CAP - t2
          if sisa <= 0 then mark_full(candX, candY); break end
          drop_try      = math.min(have, CHUNK, sisa)
          attempts_here = 0
        else
          if attempts_here >= RETRIES_TL then mark_bad(candX, candY); goto seek_slot end
...
-- ================= CAKE UTILITY (tanpa ambang minimal) =================
...
local function _ensure_single_item_in_storage(item_id, keep, storageW, storageD, opts)
  ...
  local PATH_TRY   = opts.path_try or 10
  local MAX_COLS   = opts.max_cols or 99
  ...
  local sx, sy = (function()
    ...
    local mx, my = nil, nil; if me then mx, my = math.floor(me.posx/32), math.floor(me.posy/32) end
    return mx or 1, my or 1
  end)()
  local cursor = { x = sx + 1, y = sy }
  local dir    = "right"
  ...
  while extras > 0 do
    ...
    local stanceX, stanceY = candX - 1, candY
    if stanceX < 0 or (not _is_walkable(stanceX, stanceY)) then mark_bad(candX, candY); goto seek_slot end
    if not _gotoExact(storageW, storageD, stanceX, stanceY, PATH_TRY, STEP_MS) then mark_bad(candX, candY); goto seek_slot end
    faceSide2()

    local total, stacks = _countOnTile(candX, candY)
    if (total >= TILE_CAP) or (stacks >= STACK_CAP) then mark_full(candX, candY); goto seek_slot end
    ...
    while drop_try > 0 and extras > 0 do
      ...
      if after < before then
        ...
        if sisa_cap <= 0 then mark_full(candX, candY); break end
...
function TAKE_MAGNI(WORLD, DOOR)
  ...
  -- Early: kalau sudah >1, normalisasi ke 1 dan wear sekali.
  do
  ...
  if inv:getItemCount(TARGET_ID) == 0 then
    ...
    if inv:getItemCount(TARGET_ID) == 0 then
      print("[TAKE_MAGNI] Gagal ambil 10158 (MAGNI).")
      return false
    end
  end

  -- Normalisasi lagi (kalau kebetulan double)
  local storageW, storageD = STORAGE_MAGNI, DOOR_MAGNI
  if (storageW or "") == "" then storageW, storageD = WORLD, DOOR end
  _ensure_single_item_in_storage(TARGET_ID, 1, storageW, storageD, {})

  -- Wear sekali (hindari wear dua kali)
  if inv:getItemCount(TARGET_ID) > 0 then b:wear(TARGET_ID); sleep(300); return true end
  return false
end

-- ================= FARM =================
local function faceSide2_public() faceSide2() end -- alias
...
local function HARVEST_PASS(FARM_WORLD, FARM_DOOR, farmListActive)
  ...
  if USE_MAGNI then
    ...
    if (not called_take) and inv:getItemCount(10158) > 0 then b:wear(10158); sleep(200) end
  end
...
local function RUN_MULTI_HARVEST(list_world)
  ...
  for idx, entry in ipairs(list_world or {}) do
    ...
    if W and BID then
      ...
      if not ok then
        print(("⏭ Skip %s: %s"):format(W, tostring(reason)))
        goto continue
      end
      local t0 = os.time()
...
local function RECONCILE_QUEUE()
  -- hapus world yang sudah done dari worlds
  ...
  for _, ln in ipairs(worlds) do
    ...
    if W ~= "" and (not S_done[W]) then table.insert(out, ln) end
  end
...
local function MARK_DONE(world, reason)
  ...
  _write_lines(JOB_FILES.inprogress, out)
  -- tulis ke done (dengan reason opsional)
  if reason and reason ~= "" then
    _append_line(JOB_FILES.done, string.format("%s|%s|%d|%s", world, WORKER_ID, _now(), tostring(reason)))
  else
    _append_line(JOB_FILES.done, string.format("%s|%s|%d", world, WORKER_ID, _now()))
  end
  -- keluarin dari worlds juga (biar gak balik lagi)
  RECONCILE_QUEUE()
end
...
local function PICK_ASSIST_WORLD(mode)
  mode = (mode or ASSIST_MODE or "stale"):lower()
  if not STEAL_HELP then return nil, false end
  ...
  if mode == "always" then
    return best, false -- co-op tanpa steal
  end

  if best_age >= STALE_SEC then
    local rows = _read_lines(JOB_FILES.inprogress)
...
function RUN_FROM_TXT_QUEUE()
  ...
  if resumeW then
    ...
    if W and BID then
      ...
      -- Cek warp dulu; kalau fail, close world biar gak stuck
      local ok, reason = warp_ok_and_public(W, D)
      if not ok then
        print(("[RESUME] warp gagal (%s), mark done."):format(tostring(reason)))
        MARK_DONE(W, "resume_"..tostring(reason))
      else
        HARVEST_UNTIL_EMPTY(W, D, SW, SD, {ITEM_BLOCK_ID, ITEM_SEED_ID}, hb)
        MARK_DONE(W, "resume_ok")
        print(string.format("[RESUME] %s selesai %s", WORKER_ID, W))
      end
    else
      print(string.format("[RESUME] Spec %s tidak ada di worlds.txt; unclaim.", tostring(resumeW)))
      UNCLAIM(resumeW)
    end
  end

  while true do
    -- 1) Claim job baru dulu kalau ada
    ...
    if job then
      ...
      if W and BID then
        ...
        local ok, reason = warp_ok_and_public(W, D)
        if not ok then
          print(("[JOB] warp gagal (%s), mark done."):format(tostring(reason)))
          MARK_DONE(W, "claim_"..tostring(reason))
        else
          HARVEST_UNTIL_EMPTY(W, D, SW, SD, {ITEM_BLOCK_ID, ITEM_SEED_ID}, hb)
          MARK_DONE(W, "claim_ok")
          print(string.format("[JOB] %s selesai %s", WORKER_ID, W))
        end
      end

    else
      -- 2) Tidak ada job baru bisa diklaim
      ...
      if qs.unclaimed > 0 then
        ...
        -- semua world sudah diklaim / selesai -> boleh assist sesuai ASSIST_MODE
        local assistW, stolen = PICK_ASSIST_WORLD(ASSIST_MODE)
        if assistW then
          print(string.format("[HELP] %s bantu %s (%s)", WORKER_ID, assistW, stolen and "steal" or "coop"))
          local worlds = _read_lines(JOB_FILES.worlds)
          for _, ln in ipairs(worlds) do
            local W, D, BID, SW, SD = _parse_world_line(ln)
            if W == assistW then
              ...
              local ok, reason = warp_ok_and_public(W, D)
              if not ok then
                print(("[HELP] warp gagal (%s)."):format(tostring(reason)))
                if stolen then MARK_DONE(W, "help_"..tostring(reason)) end
              else
                HARVEST_UNTIL_EMPTY(W, D, SW, SD, {ITEM_BLOCK_ID, ITEM_SEED_ID}, hb)
                -- Tutup bila kita owner (stolen) agar bersih
                if stolen then MARK_DONE(W, "help_ok") end
                print(string.format("[HELP] %s bantu selesai %s", WORKER_ID, W))
              end
              break
            end
          end
          ...
          if qs2.total == qs2.done then
            print("[QUEUE] Semua selesai. Keluar.")
            break
          end
...
do
  ...
  print(string.format("[CONFIG] ASSIGN_MODE=%s | USE_TXT_QUEUE=%s | ASSIST_MODE=%s | WORKER=%s",
    ASSIGN_MODE, tostring(USE_TXT_QUEUE), tostring(ASSIST_MODE), WORKER_ID))
