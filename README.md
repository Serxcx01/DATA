...
STORAGE_CAKE, DOOR_CAKE = "fenceblock100", "lmaokang767"
...
local WORLD_MAX_X, WORLD_MAX_Y = 99, 23 -- map kecil: x:0..99, y:0..23
...
function DROP_ITEMS_SNAKE(WORLD, DOOR, ITEMS, opts)
  ...
  for _, ITEM in pairs(ITEMS) do
    ...
    while have>0 do
      ...
      if not candX then print("[DROP] Area penuh atau habis jangkauan (kanan & kiri)."); return end
      ...
      if (total>=TILE_CAP) or (stacks>=STACK_CAP) then
        mark_full(candX,candY); cursor.x=math.min(candX+1,WORLD_MAX_X); goto seek_slot
      end

      local cap=math.max(0,TILE_CAP-total)
      local drop_try=math.min(have,CHUNK,cap)
      if drop_try<=0 then mark_full(candX,candY); cursor.x=math.min(candX+1,WORLD_MAX_X); goto seek_slot end
      ...
      while drop_try>0 and have>0 do
        ...
        if after<before then
          ...
          if sisa<=0 then mark_full(candX,candY); cursor.x=math.min(candX+1,WORLD_MAX_X); break end
