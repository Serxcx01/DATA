...
local function _idle_housekeeping()
  ...
  if (STORAGE_CAKE or "") ~= "" and has_any_cake and has_any_cake() then
    pcall(function()
      DROP_ITEMS_SNAKE(STORAGE_CAKE, DOOR_CAKE, cakeList, {tile_cap=4000,stack_cap=20,try_left_fallback = true})
    end)
  end
...
-------------------- DROP ITEMS (SNAKE) --------------------
-- ====== DROP ITEMS (SNAKE) dengan batas dinamis dari posisi bot ======
function DROP_ITEMS_SNAKE(WORLD, DOOR, ITEMS, opts)
  ...
  local MAX_COLS    = opts.max_cols or 80      -- permintaan user
  local MAX_ROWS    = opts.max_rows or 23      -- permintaan user
  local WORLD_W     = opts.world_width or 199  -- lebar world (index maksimum X)
  local TILE_CAP    = opts.tile_cap or 4000
  local STACK_CAP   = opts.stack_cap or 20
  local RETRIES_TL  = opts.tile_retries or 2
  local RIGHT_CAP   = opts.hard_right_cap or 99  -- <= batas absolut “ke kanan”
  local UP_CAP      = opts.hard_up_cap    or 23  -- <= batas absolut “ke atas”
  local TRY_LEFT_IF_FULL = (opts.try_left_fallback ~= false) -- default: true
  ...
  -- ==== Clamp area scan terhadap posisi bot ====
  -- “ke kanan” dari posisi bot: minimal(permintaan, RIGHT_CAP, sisa ruang world di kanan)
  local space_right = math.max(0, (WORLD_W - (sx + 1)))       -- dari (sx+1) s/d WORLD_W
  local eff_cols_right = math.max(0, math.min(MAX_COLS, RIGHT_CAP, space_right))

  -- “ke atas” dari posisi bot: minimal(permintaan, UP_CAP, tinggi ke atas yg tersedia)
  local space_up = math.max(0, sy)                            -- dari sy turun ke 0, jadi ada 'sy' langkah naik
  local eff_rows_up = math.max(0, math.min(MAX_ROWS, UP_CAP, space_up))

  -- helper: 1x siklus scan ke kanan (snake)
  local function scan_right_once(item_id)
    local have = b:getInventory():getItemCount(item_id)
    while have > 0 do
      ::seek_slot_right::
      local candX, candY
      -- gunakan EFEKTIF batas yang sudah di-clamp
      candX, candY, cursor = _nextDropTileSnake(sx, sy, cursor, eff_cols_right, eff_rows_up, WORLD_W, TILE_CAP, STACK_CAP)
      if not candX then
        return have  -- habis area kanan
      end
      ...
      while drop_try > 0 and have > 0 do
        ...
        if after < before then
          ...
          if (t2 >= TILE_CAP) or (s2 >= STACK_CAP) then
            mark_full(candX, candY); cursor.x = candX + 1; break
          end
  ...
  -- optional: fallback scan ke kiri kalau area kanan “habis”
  local function scan_left_once(item_id)
    -- buat kursor “mirror” ke kiri
    local curL = { x = sx - 1, y = sy }
    -- hitung ruang kiri & clamp supaya tidak lewat batas kiri/world
    local space_left = math.max(0, sx)                        -- dari 0..sx-1
    local eff_cols_left = math.max(0, math.min(MAX_COLS, RIGHT_CAP, space_left))
    local have = b:getInventory():getItemCount(item_id)
    while have > 0 do
      ::seek_slot_left::
      -- versi kiri: kita “palsukan” dengan menggeser startX jadi “mirror”, dan saat pathing pakai stanceX = candX + 1
      -- Supaya tidak nulis fungsi snake baru, kita akali pakai MAX_COLS kecil & geser cursor mundur.
      -- Cara paling aman: iter manual tile kiri ke kanan negatif—tapi tetap hemat CPU.
      ...
      if not foundX then
        return have  -- kiri pun habis
      end

      local stanceX, stanceY = foundX + 1, foundY  -- berdiri di kanan tile target (karena target di kiri kita)
      if stanceX > WORLD_W then
        mark_bad(foundX, foundY); goto seek_slot_left
      end
      if not _gotoExact(WORLD, DOOR, stanceX, stanceY, PATH_TRY, STEP_MS) then
        mark_bad(foundX, foundY); goto seek_slot_left
      end
      faceSide2() -- tetap menghadap kanan, drop ke kiri masih diterima oleh engine (item jatuh ke tile target)

      local total, stacks = _countOnTile(foundX, foundY)
      if (total >= TILE_CAP) or (stacks >= STACK_CAP) then
        mark_full(foundX, foundY); goto seek_slot_left
      end
  ...
  for _, ITEM in pairs(ITEMS) do
    -- 1) coba kanan dulu (bounded)
    local sisa = scan_right_once(ITEM)

    -- 2) kalau masih ada dan diizinkan, coba kiri (bounded)
    if sisa > 0 and TRY_LEFT_IF_FULL then
      sisa = scan_left_once(ITEM)
    end

    -- 3) kalau masih sisa -> area penuh/habis jangkauan
    if sisa > 0 then
...
function HARVEST_UNTIL_EMPTY(FARM_WORLD,FARM_DOOR,STORAGE_WORLD,STORAGE_DOOR,farmListActive,on_tick)
  ...
  while true do
    ...
    if checkitemfarm(farmListActive) then
      DROP_ITEMS_SNAKE(STORAGE_WORLD,STORAGE_DOOR,farmListActive,{tile_cap=4000,stack_cap=20,try_left_fallback = true})
      _maybe_drop_cake()
    end
...
function RUN_MULTI_HARVEST(list_world)
  ...
  for _,spec in ipairs(world_specs) do DROP_ITEMS_SNAKE(spec.storageW,spec.storageD,spec.farmItems,{tile_cap=4000,stack_cap=20,try_left_fallback = true}) end
  ...
  if (STORAGE_CAKE or "")~="" and has_any_cake() then pcall(function() DROP_ITEMS_SNAKE(STORAGE_CAKE,DOOR_CAKE,cakeList,{tile_cap=4000,stack_cap=20,try_left_fallback = true}) end) end
